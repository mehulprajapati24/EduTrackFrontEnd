...
const FacultyChatbot = () => {
  ...
  const getSchedule = async () => {
    try {
      ...
      if (token) {
        const response = await axios.get('https://edutrackbackend-opga.onrender.com/faculty/getSchedule', {
          headers: {
  ...
  const getShiftsOfFaculty = async () => {
    try {
      ...
      if (token) {
        const response = await axios.get("https://edutrackbackend-opga.onrender.com/faculty/getShiftsOfFaculty", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchStudentLocation = async () => {
    try {
      const response = await axios.get('https://edutrackbackend-opga.onrender.com/admin/get-students-location'); // Adjust API endpoint as needed
  ...
  const getProfile = async () => {
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get("https://edutrackbackend-opga.onrender.com/faculty/getProfile", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
...
if(timetableData){
  ...
  const handleSendMessage = async () => {
    if (input.trim()) {
      ...
      const responseLocation = await axios.post('https://edutrackbackend-opga.onrender.com/admin/get-student-location-based-on-prompt',{
        enrollment,
        name,
        time,
        day
