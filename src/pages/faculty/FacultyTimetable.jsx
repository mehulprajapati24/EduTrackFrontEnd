...
const FacultyTimetable = () => {
  ...
  const fetchTimetableDataForDay = async (day) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edutrackbackend-42jx.onrender.com/faculty/getFacultyTimetableBasedOnDay?day=${day}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchTimetableDataForTime = async (time) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edutrackbackend-42jx.onrender.com/faculty/getFacultyTimetableBasedOnTime?day=${selectedDay}&time=${time}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
