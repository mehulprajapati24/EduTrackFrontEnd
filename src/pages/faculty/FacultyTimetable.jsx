...
const FacultyTimetable = () => {
  ...
  const fetchTimetableData = async () => {
    ...
    try {
      ...
      if (token) {
        const response = await axios.get("https://edutrackbackend-itns.onrender.com/faculty/getFacultyTimetable", {
          headers: {
  ...
  const fetchTimetableDataForDay = async (day) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edutrackbackend-itns.onrender.com/faculty/getFacultyTimetableBasedOnDay?day=${day}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchTimetableDataForTime = async (time) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edutrackbackend-itns.onrender.com/faculty/getFacultyTimetableBasedOnTime?day=${selectedDay}&time=${time}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
