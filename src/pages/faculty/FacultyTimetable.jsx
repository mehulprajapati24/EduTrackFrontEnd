...
const FacultyTimetable = () => {
  ...
  const fetchTimetableDataForDay = async (day) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edu-track-backend-ten.vercel.app/faculty/getFacultyTimetableBasedOnDay?day=${day}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchTimetableDataForTime = async (time) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      if (token) {
        const response = await axios.get(`https://edu-track-backend-ten.vercel.app/faculty/getFacultyTimetableBasedOnTime?day=${selectedDay}&time=${time}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
