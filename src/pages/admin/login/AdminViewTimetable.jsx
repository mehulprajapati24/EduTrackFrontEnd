...
const SelectClassOrFaculty = () => {
  ...
  useEffect(() => {
    const fetchTimetableOptions = async () => {
      try {
        const response = await axios.get('http://localhost:5000/admin/get-timetables-sheetname');
  ...
  const handleTimetableSelection = async (selectedSheetName) => {
    ...
    try {
      const response = await axios.get('http://localhost:5000/admin/get-timetable-basedOnSheetName', {
        params: {
  ...
  const handleDaySelection = async (selectedDay) => {
    ...
    try {
      const response = await axios.get('http://localhost:5000/admin/get-daywise-timetable', {
        params: {
  ...
  useEffect(() => {
    if (selectedTime && selectedDay && selectedTimetable) {
      const fetchTimetableBasedOnTime = async () => {
        ...
        try {
          const response = await axios.get('http://localhost:5000/admin/get-timetable-based-on-time', {
            params: {
              sheetName: selectedTimetable,
              day: selectedDay,
              time: selectedTime,
